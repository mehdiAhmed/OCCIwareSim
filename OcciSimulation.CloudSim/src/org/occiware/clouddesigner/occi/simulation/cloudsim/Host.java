...
public class Host {
	...
	public boolean vmCreate(Vm vm) {
		...
		//System.out.println(vm.getCurrentAllocatedRam()+" >>>> "+vm.getCurrentRequestedRam());
		if (getStorage() < vm.getSize()) {
	...
	//Create an elastic VM inside Host
	public boolean vmElasticCreate(Vm vm) {
		storageConsumed.put(vm.getId(), vm.getSize());
		//System.out.println(vm.getCurrentAllocatedRam()+" >>>> "+vm.getCurrentRequestedRam());
		...
		//Elasticity for RAM
		if (!getRamProvisioner().allocateRamForVm(vm, vm.getCurrentRequestedRam())) {
			if(vm.getHost().getElasticity() == ElasticityType.NONE) //Not elastic
			{	Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by RAM");
				CloudSim.error=true;
				return false;
			}else { 
				if(vm.getHost().getElasticity() == ElasticityType.HORIZONTAL)//Horizontal
				{
					Log.printLine("VM is horizontal elastic");
					datacenter.getHostList().add(vm.getHost());
				}else { //Vertical Resize
					Log.printLine("VM is vertical elastic");
					this.setRam(vm.getCurrentAllocatedRam());
				}
	...
	public Enumerator getElasticity() {
		return this.elastic;
	}
